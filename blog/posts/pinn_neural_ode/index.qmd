...
This shift, introduced as **Neural Ordinary Differential Equations** by Chen et al. [@chen2018], leads to three methods with different inductive biases (compare with [@raissi2019] for the PINN formulation):
...
**Real-data application** — Dandekar and Barbastathis [@dandekar2020] applied a UDE-like approach to COVID-19 province-level case counts, learning a quarantine-strength function alongside the SIR skeleton. The approach recovered interpretable intervention signals consistent with known lockdown dates — demonstrating that the partial-physics architecture generalises from simulated to noisy real-world data.

**Comprehensive reference** — Kidger [@kidger2022] provides a unified treatment of all neural differential equation variants — ODEs, stochastic DEs, and controlled DEs — including the adjoint method, theoretical existence results, and training pathologies. Recommended for readers moving beyond the tutorial level.

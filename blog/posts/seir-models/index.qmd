...
SEIR 모형은 잠복기가 어느 정도 긴 감염병 (예를 들어 코로나19)의 전파를 모형하는 데 사용한다 [@hethcote2000; @keeling2008]. 이번 포스트에서는 SEIR 모형을 만드는 방법을 알아본다. 결정론적 (deterministic) 그리고 확률론적 (stochastic) 방법으로 SEIR 모형을 R언어로 만들어 본다.
...
위에서 기술한 확률론적 방법은 우리가 이미 정한 time interval $\delta$에 따라 오차가 발생하는 반면 Gillespie algorithm [@gillespie1977] 을 이용해서 통계적으로 정확한 stochastic simulation 을 할 수 있다.

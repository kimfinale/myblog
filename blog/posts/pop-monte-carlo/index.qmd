...
최근에 파티클필터링 (particle filtering; PF) 방법을 이용하여 $\mathcal{R}_t$ 추정하는 과정에 대한 논문을 썼다. 그런데, 항상 의문이었던 것은 PF를 조금만 변형하면 감염병 모형의 감염속도 $\beta=\mathcal{R}_0 \gamma$ 와 같은 time-invariant 파라미터를 추정할 수도 있지 않을까 하는 것이었다. Population Monte Carlo (PMC) [@cappe2004] 가 바로 그 방법이었다.

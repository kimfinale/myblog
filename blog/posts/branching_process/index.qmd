...
In the branching process model, the number of secondary infections is realized as a random number (e.g., Poisson or negative binomial) [@harris1963]. This framework is especially useful for studying outbreak probability and final epidemic size.
...
set.seed(42)
R0_mean <- 2
R0_size <- 0.2 # loosely based on the estimate for Ebola (see Kucharski et al. 2016 [@kucharski2016])
popsize = 1000 # population size for each iteration
nrun <- 1000 # number of iterations to compute the mean
outbreaks <- rep(NA, nrun) # to store outbreak size for each iteration
init_inf <- 1 # initially infected people

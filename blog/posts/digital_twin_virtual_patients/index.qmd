...
date: "2026-04-22"
categories: [digital twin, vaccine, virtual patients, Latin hypercube sampling, R]
...
```{r lhs-demo, fig.cap="Latin hypercube sampling (LHS, orange) vs simple random sampling (SRS, blue) for two parameters, n = 50. LHS guarantees that each row of each parameter's range is occupied exactly once (visible as an even spread along both axes). SRS often leaves gaps in some regions and clusters in others. In high dimensions, this coverage advantage becomes decisive."}
set.seed(42)
n_demo <- 50
...
df_srs <- srs_2d(n_demo) |> mutate(method = "SRS")
df_cmp <- bind_rows(df_lhs, df_srs)

ggplot(df_cmp, aes(x = p1, y = p2, colour = method)) +
  geom_point(size = 2, alpha = 0.8) +
  facet_wrap(~method) +
  scale_colour_manual(values = c("LHS" = "#E87722", "SRS" = "#3B6EA8"),
                      guide = "none") +

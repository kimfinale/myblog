...
The left panel shows the B-PINN variational posterior predictive: $M = 500$ samples
$(\beta^{(m)}, \gamma^{(m)}) \sim q$ are drawn, the SIR ODE is integrated for each,
and percentiles form the credible band. The right panel shows an **Ensemble PINN**
alternative: $n = 10$ independent point-estimate PINNs are trained from different
random seeds, and their converged $(\hat\beta, \hat\gamma)$ values are treated as
implicit posterior samples.
...
I_lo_e  = np.percentile(traj_ens[:, :, 1], 2.5,  axis=0)
I_med_e = np.percentile(traj_ens[:, :, 1], 50,   axis=0)
I_hi_e  = np.percentile(traj_ens[:, :, 1], 97.5, axis=0)
...
for ax, lo, med, hi, title in [
    (axes[0], I_lo,   I_med,   I_hi,   "B-PINN (variational, M=500)"),
    (axes[1], I_lo_e, I_med_e, I_hi_e, f"Ensemble PINN (n={n_ens})"),
]:
    ax.fill_between(t_viz_np, lo, hi, alpha=0.25, color=C_PI,
                    label="95% PI")
    ...
    ax.scatter(t_obs, I_obs, color=C_OBS, s=15, zorder=5, label="Observed")
...
::: {.callout-note}
The B-PINN credible band is wider because the mean-field variational family misses the
negative $\beta$–$\gamma$ correlation: high-$\beta$/high-$\gamma$ and
low-$\beta$/low-$\gamma$ pairs both fit the data but produce very different trajectories,
inflating the envelope. The Ensemble PINN implicitly selects only parameter combinations
that individually fit the data, giving a tighter and better-calibrated band.
:::

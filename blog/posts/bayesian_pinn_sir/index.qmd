...
I_lo_h  = np.percentile(traj_hmc[:, :, 1], 2.5,  axis=0)
I_med_h = np.percentile(traj_hmc[:, :, 1], 50,   axis=0)
I_hi_h  = np.percentile(traj_hmc[:, :, 1], 97.5, axis=0)
```
...
for ax, lo, med, hi, title in [
    (axes[0], I_lo,   I_med,   I_hi,   "B-PINN VI (M=500)"),
    (axes[1], I_lo_h, I_med_h, I_hi_h, f"HMC (n={n_samp_h})"),
    (axes[2], I_lo_e, I_med_e, I_hi_e, f"Ensemble PINN (n={n_ens})"),
]:
    ax.fill_between(t_viz_np, lo, hi, alpha=0.25, color=C_PI, label="95% PI")
...
::: {.callout-note}
**Why HMC outperforms mean-field VI**
...
HMC explores this ridge correctly via gradient-driven leapfrog proposals; VI inflates
the marginal variances to compensate for the missing correlation, widening the
predictive envelope. The Ensemble PINN bypasses the posterior entirely — each PINN

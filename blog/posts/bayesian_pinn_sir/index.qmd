...
All three panels show the **full posterior predictive for observed counts**: each
...
The left panel uses $M = 500$ samples $(\beta^{(m)}, \gamma^{(m)}) \sim q$ from the
variational posterior. The middle uses 500 HMC draws. The right panel draws 500
replicates from the 10 ensemble members with added observation noise.

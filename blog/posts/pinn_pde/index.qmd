...
The natural continuous-space extension is a **reaction-diffusion system** — a PDE that combines local transmission (reaction) with spatial movement of infectious individuals (diffusion) [@murray2003; @noble1974].
...
Adding spatial diffusion to the SIR equations [@kermack1927; @hethcote2000] in fraction form gives a system of PDEs on $x \in [0, L]$, $t \in [0, T]$:
...
The resulting dynamics are a **travelling wave** of infection propagating outward from the source [@murray2003b].
The wave speed is approximately $c_w \approx 2\sqrt{D(\beta - \gamma)}$ — a result analogous to Fisher's travelling wave [@fisher1937; @murray2003b], depending on both diffusion and the net growth rate $\beta - \gamma$.
...
The spatiotemporal PINN extends the framework of Raissi et al. [@raissi2019] from ODEs to PDEs, following the same collocation-point approach used in subsurface flow [@tartakovsky2020] and other PDE inverse problems.

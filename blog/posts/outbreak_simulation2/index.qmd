...
$$\tilde N \to \tilde t \to \tilde D \to \tilde i$$

Population size, $\tilde{N}_n$
...
Outbreak duration (weeks), $\tilde{D}_n$
$$
...
Peak weekly incidence per person, $\tilde{i}_n$
...
outbk <- tibble(year, region, country, N_pop, t_peak_weeks, 
                duration_weeks, i_peak_per_person)
...
# transformed pairwise structure (clamp i ∈ (0,1) before qlogis)
GGally::ggpairs(
  outbk |>
    transmute(
      logN   = log(N_pop),
...
# expected directionality (quick numerical checks)
with(outbk, {
  cat("cor(logN, logt)       =", 
...
$$
# Estimating parameters using the simulated data

Design matrices (splines + interactions) for a regression model in Stan
...
# --- p(logN): splines of year only with 8 degrees of freedom
...
# Data
```{r}
data <- data.table::fread("C:\\Users\\jonghoon.kim\\Workspace\\myblog\\data\\outbreak_data_summary_20251022.csv")
data$i_peak_per_person <- data$peak_week_incidence_per_10e3/10e3
  
cols <- c("ar_per_10e3", "outbreak_pop", "total_s_ch", "duration_weeks", 
          "peak_week_incidence_per_10e3", "time_to_peak_week")
...
# basic marginals
data |>
  pivot_longer(c(outbreak_pop, time_to_peak_week, duration_weeks, i_peak_per_person)) |>
  ggplot(aes(value)) + 
  geom_histogram(bins = 30, fill = "grey70") +
  facet_wrap(~name, scales = "free") + 
  labs(title = "Marginals (nested generator)")
...
GGally::ggpairs(
  data |>
    transmute(
      logN   = log(outbreak_pop),
      logt   = log(time_to_peak_week),
      logD   = log(duration_weeks),
      logit_i = qlogis(pmin(pmax(i_peak_per_person, 1e-6), 1 - 1e-6))
    ),
  title = "Transformed pairwise (nested)"

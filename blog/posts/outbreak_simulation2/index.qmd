...
This self-contained file implements your **ordered conditional** approach 
using **plain Stan + rstan** (no `brms`).
...
2. Build **design matrices** (splines + interactions) entirely in R.
3. Fit a **single Stan model** with four linked regressions (on transformed scales) and **correlated region/country random effects** (multivariate for the 4 outcomes).
4. **Validate** with posterior predictive checks.
5. **Simulate new outbreaks** forward in your modeling order (\\(\\log N \\to \\log t \\to \\log D \\to \\text{logit}(i_\\text{peak})\\)).
...
# 1) Generate Fake Historical Outbreaks
...
build_map_unbalanced <- function(R, C, conc = 1) {
  regs <- paste0("R", seq_len(R))
  w    <- rgamma(R, shape = conc, rate = 1); w <- w / sum(w)
...
# Choose one:
# country_region_map <- build_map_unbalanced(R, C, conc = 0.7)  # or build_map_balanced(R, C)
...
logit_i <- logit_i_mu + re_region[4, r_id] + re_country[4, c_id] + rnorm(n_outbreaks, 0, 0.35)
...
outbk |>
  pivot_longer(c(N_pop, t_peak_weeks, duration_weeks, i_peak_per_person)) |>
  ggplot(aes(value)) + geom_histogram(bins = 30, fill = "grey70") +
  facet_wrap(~name, scales = "free") + labs(title = "Marginals (nested generator)")
...
with(outbk, {
  cat("cor(logN, logt)       =", cor(log(N_pop), log(t_peak_weeks)), "\n")
  cat("cor(logN, logD)       =", cor(log(N_pop), log(duration_weeks)), "\n")
  cat("cor(logN, logit(i))   =", cor(log(N_pop), qlogis(pmin(pmax(i_peak_per_person,1e-6),1-1e-6))), "\n")
  cat("cor(logt, logD)       =", cor(log(t_peak_weeks), log(duration_weeks)), "\n")
})
```

---

# 2) Design Matrices (splines + interactions)
...
drop_bad_cols <- function(M) {
  ...
  if (!any(keep)) stop("All columns were dropped as 'bad' in a design matrix. Reduce df or check inputs.")
  M[, keep, drop = FALSE]
}
...
stopifnot(all(c("year","region","country","logN","logt","logD","logit_i") %in% names(df)))
...
# if (has_zero_var(XN)) stop("XN has zero-variance columns after cleaning.")
# if (has_zero_var(Xt)) stop("Xt has zero-variance columns after cleaning.")
# if (has_zero_var(XD)) stop("XD has zero-variance columns after cleaning.")
# if (has_zero_var(XI)) stop("XI has zero-variance columns after cleaning.")
...
if (has_zero_var(XN, intercept = TRUE)) stop("XN has zero-variance columns (excluding intercept).")
if (has_zero_var(Xt, intercept = TRUE)) stop("Xt has zero-variance columns (excluding intercept).")
if (has_zero_var(XD, intercept = TRUE)) stop("XD has zero-variance columns (excluding intercept).")
if (has_zero_var(XI, intercept = TRUE)) stop("XI has zero-variance columns (excluding intercept).")
...
---
# 3) Stan model and fit

```{r}
## ---------- Fit the simpler diagonal-RE Stan model ----------
# Ensure the file 'ordered_conditional_plain_diagRE.stan' exists in the working directory
stan_code <- "
data {
  int<lower=1> N;
  int<lower=1> R;
...
ppc_dens_overlay(y = Yobs$logN,   yrep = Yreps$logN)   + ggtitle("PPC: logN (density)")
ppc_dens_overlay(y = Yobs$logt,   yrep = Yreps$logt)   + ggtitle("PPC: log t_peak (density)")
ppc_dens_overlay(y = Yobs$logD,   yrep = Yreps$logD)   + ggtitle("PPC: log duration (density)")
ppc_dens_overlay(y = Yobs$logiti, yrep = Yreps$logiti) + ggtitle("PPC: logit i_peak (density)")

ppc_ecdf_overlay(y = Yobs$logN,   yrep = Yreps$logN)   + ggtitle("PPC: logN (ECDF)")
ppc_ecdf_overlay(y = Yobs$logt,   yrep = Yreps$logt)   + ggtitle("PPC: log t_peak (ECDF)")
ppc_ecdf_overlay(y = Yobs$logD,   yrep = Yreps$logD)   + ggtitle("PPC: log duration (ECDF)")
ppc_ecdf_overlay(y = Yobs$logiti, yrep = Yreps$logiti) + ggtitle("PPC: logit i_peak (ECDF)")
...
---
# 5) Forward Simulation in Your Modeling Order
...
simulate_forward <- function(n_sims = 400,
  ...
  stopifnot(length(year) == n_sims, length(region) == n_sims, length(country) == n_sims)
...
sim_mix <- simulate_forward(n_sims = nrow(keys),
                            year = keys$year,
...
sim_summ <- sim_mix %>% mutate(source = "simulated")
...
# # -------- Gamma-renewal curve -------------
# # Discrete Gamma generation time -> weights w (sum to 1)
# # --- safer discrete Gamma weights (no NAs, sum to 1) ---
# gt_weights <- function(max_lag = 40, mean = 5, sd = 2) {
#   mean <- pmax(mean, 1e-6); sd <- pmax(sd, 1e-6)
#   k <- (mean / sd)^2
#   theta <- sd^2 / mean
#   lags <- 1:max_lag
#   # CDF diffs; clamp to [0,1] then renormalize
#   lo <- pmax(pgamma(lags - 0.5, shape = k, scale = theta), 0)
#   hi <- pmin(pgamma(lags + 0.5, shape = k, scale = theta), 1)
#   dens <- pmax(hi - lo, 0)
#   s <- sum(dens)
#   if (!is.finite(s) || s <= 0) {
#     # fallback: continuous pdf at integer lags with a small mass, then normalize
#     dens <- dgamma(lags, shape = k, scale = theta)
#     s <- sum(dens)
#   }
#   if (!is.finite(s) || s <= 0) dens[] <- 0  # last resort (all zeros)
#   w <- dens / pmax(s, 1e-12)
#   # ensure finite
#   w[!is.finite(w)] <- 0
#   # final normalize
...
# }
# 
# # --- map rates to R_up / R_down with guards ---
# rt_from_rates_gamma <- function(r_up, d_down, mean = 5, sd = 2, R_cap = 30) {
#   # R_cap limits explosive growth in recursion; tune as needed
#   mean <- pmax(mean, 1e-6); sd <- pmax(sd, 1e-6)
#   k <- (mean / sd)^2
#   theta <- sd^2 / mean
# 
#   r_up  <- pmax(r_up,  0)
#   d_down<- pmax(d_down,0)
# 
#   R_up  <- (1 + theta * r_up)^k
#   # ensure (1 - theta*d) > 0
#   d_cap <- pmin(d_down, (1 - 1e-6)/theta)
#   R_down <- pmax((1 - theta * d_cap)^k, 1e-6)
# 
#   # clamp extreme Rt to keep recursion stable
#   R_up   <- pmin(R_up,   R_cap)
#   R_down <- pmax(pmin(R_down, R_cap), 1e-6)
# 
#   c(R_up = R_up, R_down = R_down)
# }
# 
# # Build per-person incidence via a renewal equation and scale to hit i_peak.
# # - R_t is piecewise-constant: R_up before 't_peak', R_down after.
# # - Seed with tiny incidence, then simulate forward, then scale the curve to match i_peak.
# inc_curve_renewal_gamma <- function(week, t_peak, i_peak, D,
#                                     a = 0.5, eps = 0.05,
#                                     gt_mean = 5, gt_sd = 2,
#                                     max_lag = 40) {
#   # derive rates from duration & asymmetry
#   rd <- rd_from_D_asym(D, a = a, eps = eps)
#   Rt <- rt_from_rates_gamma(r_up = rd["r"], d_down = rd["d"],
#                             mean = gt_mean, sd = gt_sd, R_cap = 30)
#   R_up <- Rt["R_up"]; R_down <- Rt["R_down"]
# 
#   # weights (finite, normalized)
#   w <- gt_weights(max_lag = max_lag, mean = gt_mean, sd = gt_sd)
# 
#   # align t_peak to nearest index on 'week'
#   t_idx_peak <- which.min(abs(week - t_peak))
#   Tn <- length(week)
# 
#   # piecewise-constant R_t
#   R_vec <- rep(R_down, Tn)
#   R_vec[seq_len(t_idx_peak)] <- R_up
#   R_vec[!is.finite(R_vec)] <- 1  # guard
# 
#   # recurrence
#   i <- rep(0, Tn)
#   i[1] <- 1e-8
...
#     if (lag_max <= 0) { i[t] <- 0; next }
#     # convolution sum; all finite guards
#     past <- i[(t - 1):(t - lag_max)]
#     ww   <- w[1:lag_max]
#     past[!is.finite(past)] <- 0
#     ww[!is.finite(ww)] <- 0
#     lambda <- sum(past * ww)
...
#   }
# 
#   # robust scaling to target peak
#   peak_val <- suppressWarnings(max(i, na.rm = TRUE))
...
# 
#   i_scaled <- s * i
#   # soft truncation below eps*i_peak
#   i_scaled[!is.finite(i_scaled)] <- 0
#   i_scaled[i_scaled < eps * i_peak] <- 0
#   pmin(pmax(i_scaled, 0), 0.99)
# }
...
```{r}
sim_ren_aligned <- sim_ren %>%
  mutate(rel_week = week - t_peak) %>%
  group_by(rel_week) %>%
  summarise(
    cases_mean = mean(cases),
    cases_q05  = quantile(cases, 0.05),
...
ggplot(sim_ren_aligned, aes(rel_week, cases_mean)) +
  geom_ribbon(aes(ymin = cases_q05, ymax = cases_q95), alpha = 0.2) +
  geom_line() +
  labs(title = "Gamma–renewal simulated weekly cases (aligned at peak)",
       x = "weeks relative to peak", y = "cases") +
  theme_minimal()
...
sim_asym  <- simulate_weekly_counts(sim_meta, Tmax = 50, shape_mode = "exp_asym", asym_a = 0.3)
...
sim_mix4  <- simulate_weekly_counts(sim_meta, Tmax = 60, shape_mode = "mix",
                                    mix_probs = c(exp_sym = 0.3, exp_asym = 0.3, tri = 0.2, renewal_gamma = 0.2))

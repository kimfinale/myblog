...
We consider $n$ independent populations. Each population experiences an outbreak with probability $p$ (equal risk across populations). A central stockpile can fund vaccination campaigns in at most a fraction $0 < f \le 1$ of populations (i.e., $fn$ campaigns total). Campaigns can be deployed **pre-emptively** (before observing outbreaks) and/or **reactively** (only if an outbreak occurs).

We study the large-$n$ limit so that population fractions are deterministic.

### Strategy parameterization

Let $f_{\mathrm{pre}}$ be the fraction vaccinated pre-emptively and $f_{\mathrm{react}}$ the fraction reserved for reactive use. The capacity constraint is
$$
...
A fraction $f_{\mathrm{pre}}=\alpha f$ is vaccinated regardless of whether an outbreak occurs:
$$
...
- **Reactive-rich** (enough reactive campaigns to cover all outbreaks among the non-pre-emptive group):
$$
...
- with probability $q(\alpha)$: pay $C_{\mathrm{V}} + (1-r)C_{\mathrm{I}}$,
...
- within each regime, the minimizer is attained at a **boundary** (a corner), and
...
- If $r > p$, compare the two corners $\alpha=0$ (pure reactive) and $\alpha=\alpha_c$ (largest feasible pre-emptive share while keeping the remaining group reactive-rich). The switching threshold is
$$
...
**Interpretation of $\alpha_c$.** $\alpha_c$ is **not** found by minimizing a smooth function; it is the **feasibility limit** for staying reactive-rich among the remaining populations. It is the largest pre-emptive share such that the remaining reactive stockpile can still cover all expected outbreaks in the non-pre-emptive group.

**Interpretation of $R_{\mathrm{thr}}$.** When $r>p$ and $f>p$, the choice is between:

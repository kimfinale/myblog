...
Key insights are:
  1. The cost remains constant for the pre-emptive strategy whereas it increases with outbreak probability $p$ for the reactive strategy.
  2. There is a threshold outbreak probability, $p_{\mathrm{thr}}^{(1)}$ where the cost for the pre-emptive and reactive strategies equal and under which reactive vaccination is economically prefereable. 
  3. $p_{\mathrm{thr}}^{(1)}$ is larger for higher effectiveness of the reactive strategy, $r$. 
...
df_brace1 <- data.frame(x=c(0.95,1), y=c(1,cost_react_max1))
...
ggplot(df_cost, aes(x = p, y = cost, color = strategy, 
  ...
  geom_text(
    data = df_pstar,
    aes(x = p_star, y = Inf, label = "~italic(p)[thr]^(1)"),
    parse = TRUE,
    inherit.aes = FALSE,
    hjust = -0.3,
    vjust = 1.2,
  ...
  annotate("text", x = min(df_brace1$x)-0.01, y = mean(df_brace1$y), 
           label = "Cost of\ndelayed response", 
...
```
Reactive vaccination becomes more practical 
...
-   with probability $p$: an outbreak occurs and is reactively
    vaccinated, cost $C_{\mathrm{V}} + (1-r)C_{\mathrm{I}}$;
...
## Two Risk Groups and Limited Vaccination Capacity
...
\min\left(1, \frac{f_H}{f}\right) \quad (\text{focused on high-risk}), & \text{if } p_L \le r \le p_H \\
...
\min\left(1, \frac{f_H}{f}\right) \quad (\text{focused on high-risk}), & \text{if } R_H^* \le R < R_L^* \\
...
The intermediate strategy $\alpha = \min(1, f_H/f)$ is the "Focused"
...
both groups. 2. **Limited for L:** We saturate high-risk ($f_H$).
Remaining capacity ($f-f_H$) is used reactively on low-risk but is
exhausted. Cost is total capacity $f$ + net unprevented outbreaks in
Low-Risk. 3. **Abundant for L:** We saturate high-risk ($f_H$).
Remaining capacity is sufficient to cover all Low-Risk outbreaks. Cost
is pre-emptive ($f_H$) + reactive campaigns used ($f_L p_L$) + failed
reactive attempts.

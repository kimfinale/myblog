...
The threshold $p^*$ decreases as the reactive effectiveness $r$
decreases or the outbreak cost ratio $R$ increases. Thus, pre-emptive
vaccination becomes more favorable when reactive vaccination is less
effective or when outbreaks are relatively more costly.
...
In the figure below, $c_s^{(1)}$ denotes the normalized per-population expected
cost for strategy $s \in \{\mathrm{pre}, \mathrm{react}\}$.

The cost remains constant for the pre-emptive strategy whereas it increases with probability $p$ for the reactive strategy.
...
#### $p^*$ as a function of $R$ for $r=0.5$.
...
In a single population, the normalized per-population expected cost of the pre-emptive strategy remains at 1, while the reactive strategy cost increases linearly with the outbreak probability $p$. Their intersection
defines the threshold $p^*$. Pre-emptive vaccination is favored when $r$
...
Expected cost of pre-emptive strategy $c_{\mathrm{pre}}^{(n)}$ increases as $f$ increases whereas expected cost of reactive strategy $c_{\mathrm{react}}^{(n)}$ increases only up to a point where $f=p$ and then remains constant. In other words, $c_{\mathrm{react}}^{(n)}$ is not a function of $f$ for $f>p$.

$p = 0.5, R = 0.9$
Cost increases with $f$ for $p \cdot R > 1$
...
3D surface of $p^*$ across $R$ and $r$ (equal-risk multi-population
case):
...
We define the **multi-population threshold** $p_*^{(n)}$ as the value of
$p$ at which pure pre-emptive and pure reactive strategies have equal
normalized cost: $$
c_{\mathrm{pre}}^{(n)} = c_{\mathrm{react}}^{(n)}.
$$

Because $c_{\mathrm{react}}^{(n)}$ is piecewise, $p_*^{(n)}$ also has a
piecewise form.
...
Consistency with the reactive-limited regime requires
$f \le p_*^{(n)} = r$. So, **when capacity is relatively scarce**
($f \le r$), the threshold is $$
p_*^{(n)} = r,
\qquad \text{if } f \le r.
$$

In this regime, the threshold is independent of $R$: only the relative
size of $f$ and $r$ matters.
...
Consistency with the reactive-rich regime requires $p_*^{(n)} \le f$,
which holds whenever $f \ge r$ and $R > 0$. Thus, when capacity is
relatively abundant ($f \ge r$), the threshold is
...
-   When vaccine capacity is **scarce** relative to reactive
    effectiveness ($f \le r$), the threshold does not depend on $R$ and
    is simply $p_*^{(n)} = r$.
-   When capacity is **abundant** ($f > r$), the threshold depends on
    the outbreak–to–vaccination cost ratio:
    -   larger $R$ (more costly outbreaks) $\Rightarrow$ smaller
        $p_*^{(n)}$,
    -   larger $f$ (more capacity) $\Rightarrow$ larger $p_*^{(n)}$ for
        fixed $R$ and $r$.

## Multiple Populations with Equal Risk, Limited Vaccination Capacity, and A Mixed strategy

We now allow the fixed capacity $f$ to be split between pre-emptive and
reactive use.
...
-   $\alpha = 1$: pure pre-emptive strategy ($f_{\mathrm{pre}} = f$,
    $f_{\mathrm{react}} = 0$),
-   $\alpha = 0$: pure reactive strategy ($f_{\mathrm{pre}} = 0$,
    $f_{\mathrm{react}} = f$).
...
The remaining fraction $(1 - f_{\mathrm{pre}}) = (1 - \alpha f)$ is not
pre-emptively vaccinated. Among these:
...
Two regimes arise, depending on whether reactive capacity is rich or
limited **among non-pre-emptive populations**:
...
depends on the cost ratio $R$ relative to the threshold $R^*$:
...
> ($R \ge R^*$), the mixed strategy $\alpha_* = \frac{f - p}{f(1 - p)}$
...
initially **Reactive-Rich**. Pre-emptive vaccination is only justified
if the cost of outbreaks ($R$) is high enough to outweigh the efficiency
of waiting.

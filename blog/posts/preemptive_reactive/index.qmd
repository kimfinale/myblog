...
- with probability $p$: an outbreak occurs;
    ...
    reactive vaccination, cost $C_{\mathrm{V}} + (1-r)C_{\mathrm{I}}$;
    - conditional on outbreak, with probability $1 - f/p$:
    receives no vaccine, cost $C_{\mathrm{I}}$.
...
$p = 0.4, R = 0.9$. Cost increases with $f$ for $p \cdot R > 1$
...
$p = 0.4, R = 5$. Cost decreases as $f$ increases for the costly outbreaks ($R > 1$)

...
-   $R = C_{\mathrm{O}}/C_{\mathrm{V}}$ = outbreak–to–vaccination cost ratio,
-   $f \in (0,1]$ = total vaccination capacity as a fraction of populations,
-   $\alpha \in [0,1]$ = fraction of capacity used pre-emptively,
-   $f_{\mathrm{pre}} = \alpha f$ = fraction of populations vaccinated pre-emptively,
-   $f_{\mathrm{react}} = (1-\alpha)f$ = capacity reserved for reactive campaigns.

## One Population

We consider a single population with outbreak probability $p$, outbreak cost $C_{\mathrm{O}}$, vaccination cost $C_{\mathrm{V}}$, and reactive vaccination effectiveness $r \in [0,1]$. The outbreak cost $C_{\mathrm{O}}$ reflects direct medical costs, productivity losses, and mortality-related losses. The vaccination cost $C_{\mathrm{V}}$ includes vaccine and delivery costs.
...
Under reactive vaccination, the population is vaccinated only if an outbreak occurs. If $X=1$, the vaccination cost $C_{\mathrm{V}}$ is incurred and residual outbreak costs equal $(1-r) C_{\mathrm{O}}$. If $X=0$, there is no cost. The total cost random variable is:
...
### Threshold Outbreak Probability $p_*$

The threshold outbreak probability $p_*$ at which the two strategies yield equal expected cost is given by:
...
The threshold $p_*$ decreases as the reactive effectiveness $r$ decreases or the outbreak cost ratio $R$ increases. Thus, pre-emptive vaccination becomes more favorable when reactive vaccination is less effective or when outbreaks are relatively more costly.
...
Normalized per-population costs across $p$ for varying $r$. In the figure below, $c_s^{(1)}$ denotes the normalized per-population expected cost for strategy $s \in \{\mathrm{pre}, \mathrm{react}\}$.
...
$p_*$ as a function of $r$ for varying $R$'s
...
3D surface of $p_*$ across $r$ and $R$:
...
In a single population, the normalized expected cost of the pre-emptive strategy remains at 1, while the reactive strategy cost increases linearly with the outbreak probability $p$. Their intersection defines the threshold $p_*$. Pre-emptive vaccination is favored when $r$ is low or $R$ is high. This analysis assumes no costs associated with unused vaccines, a simplifying assumption that can be relaxed in more detailed models.

## Multiple Populations with Equal Risk and Limited Vaccination Capacity

We now extend the model to $n$ independent populations, each with the same outbreak probability $p$. We assume a fixed vaccine stockpile sufficient to vaccinate only a fraction $0 < f \le 1$ of the populations (i.e., we can run $fn$ vaccination campaigns in total). We consider the large–$n$ limit so that fractions of populations behave deterministically (law of large numbers).
...
### Pure pre-emptive and pure reactive strategies

We first consider two benchmark strategies that both respect the capacity constraint.
...
-   The remaining fraction $(1 - f)$ is not vaccinated pre-emptively; each such population experiences an outbreak with probability $p$ and, under a pure pre-emptive strategy, receives **no** reactive vaccination. Outbreaks in this group cost $C_{\mathrm{O}}$.
...
-   **Vaccine-rich reactive regime** ($f \ge p$):
    ...
    Capacity is sufficient to respond to essentially all outbreaks ($fn \ge pn$). For each population,

    -   with probability $p$: an outbreak occurs and is reactively vaccinated, cost $C_{\mathrm{V}} + (1-r)C_{\mathrm{O}}$;
...
-   **Vaccine-limited reactive regime** ($f < p$):
    ...
    On average there are more outbreaks than available campaigns ($pn > fn$). Assuming reactive campaigns are allocated uniformly at random among outbreaks, the fraction of outbreaks that receive a reactive campaign is approximately $f/p$.

    For a given population:

    -   with probability $p$: an outbreak occurs;
        -   conditional on outbreak, with probability $f/p$: receives reactive vaccination, cost $C_{\mathrm{V}} + (1-r)C_{\mathrm{O}}$;\
        -   conditional on outbreak, with probability $1 - f/p$: receives no vaccine, cost $C_{\mathrm{O}}$.
...
### Visualization (equal-risk, multi-population)
...
$c_{\mathrm{pre}}^{(n)}$ and $c_{\mathrm{react}}^{(n)}$ for $p = 0.4, R = 1$:
...
$p^*$ in case of multipopulation of equal-risk for $f = 0.5$:
...
3D surface of $p^*$ across $R$ and $r$ (equal-risk multi-population case):
...
### Threshold outbreak probability $p_*^{(n)}$ (equal risk, multi-population)

We define the **multi-population threshold** $p_*^{(n)}$ as the value of $p$ at which pure pre-emptive and pure reactive strategies have equal normalized cost: $$
c_{\mathrm{pre}}^{(n)} = c_{\mathrm{react}}^{(n)}.
$$

Because $c_{\mathrm{react}}^{(n)}$ is piecewise, $p_*^{(n)}$ also has a **piecewise form**.

#### Case 1: capacity scarce or just sufficient at the threshold ($f \le p_*^{(n)}$)
...
f + (1-f)p_*^{(n)}R = f + \bigl(p_*^{(n)} - fr\bigr)R
\quad\Longrightarrow\quad
p_*^{(n)} = r.
$$

Consistency with the reactive-limited regime requires $f \le p_*^{(n)} = r$.\
So, **when capacity is relatively scarce** ($f \le r$), the threshold is $$
p_*^{(n)} = r,
\qquad \text{if } f \le r.
$$

In this regime, the threshold is **independent of** $R$: only the relative size of $f$ and $r$ matters.

#### Case 2: capacity abundant at the threshold ($f \ge p_*^{(n)}$)
...
f + (1-f)p_*^{(n)}R = p_*^{(n)}\bigl[1 + (1-r)R\bigr]
\quad\Longrightarrow\quad
p_*^{(n)} = \frac{f}{1 + R(f - r)}.
$$

Consistency with the reactive-rich regime requires $p_*^{(n)} \le f$, which holds whenever $f \ge r$ and $R > 0$. Thus, **when capacity is relatively abundant** ($f \ge r$), the threshold is $$
p_*^{(n)} = \frac{f}{1 + R(f - r)},
\qquad \text{if } f \ge r.
$$
...
### Summary (equal-risk multi-population threshold)
...
-   When vaccine capacity is **scarce** relative to reactive effectiveness ($f \le r$), the threshold does not depend on $R$ and is simply $p_*^{(n)} = r$.\
-   When capacity is **abundant** ($f > r$), the threshold depends on the outbreak–to–vaccination cost ratio:
    -   larger $R$ (more costly outbreaks) $\Rightarrow$ smaller $p_*^{(n)}$,
    -   larger $f$ (more capacity) $\Rightarrow$ larger $p_*^{(n)}$ for fixed $R$ and $r$.
...
-   $\alpha = 1$: pure pre-emptive strategy ($f_{\mathrm{pre}} = f$, $f_{\mathrm{react}} = 0$),
-   $\alpha = 0$: pure reactive strategy ($f_{\mathrm{pre}} = 0$, $f_{\mathrm{react}} = f$).
...
-   with probability $p$: outbreak occurs and is reactively vaccinated, cost $C_{\mathrm{V}} + (1-r)C_{\mathrm{O}}$;
-   with probability $(1-p)$: no outbreak, no cost.
...
When capacity exceeds the expected outbreak size, the optimal strategy depends on the cost ratio $R$ relative to the threshold $R^*$:

$$
\alpha^* =
\begin{cases}
1 \quad (\text{pure pre-emptive}), & \text{if } r < p \\
0 \quad (\text{pure reactive}), & \text{if } r > p \text{ and } R < R^* \\
\frac{f - p}{f(1 - p)} \quad (\text{mixed strategy}), & \text{if } r > p \text{ and } R \ge R^*
\end{cases}
$$

> **Note:** In the abundant regime with high outbreak costs ($R \ge R^*$), the mixed strategy $\alpha_* = \frac{f - p}{f(1 - p)}$ ensures that pre-emptive vaccination is maximized just enough so that the remaining capacity can fully cover the remaining risk reactively.
...
> **Interpretation:**
> *   **$r < p_L$**: Reactive vaccination is weak. Pre-empt everyone possible.
> *   **$p_L \le r \le p_H$**: Reactive is better than pre-empting Low-Risk, but worse than pre-empting High-Risk. Prioritize High-Risk.
> *   **$r > p_H$**: Reactive is highly effective. Wait and respond to outbreaks.
...
> **Interpretation:**
> *   **$R < R_H^*$**: Outbreaks are cheap. Use pure reactive strategy.
> *   **$R_H^* \le R < R_L^*$**: Outbreaks are costly enough to pre-empt High-Risk, but Low-Risk is still effectively managed reactively.
> *   **$R \ge R_L^*$**: Outbreaks are catastrophic. Pre-empt everyone possible.



### Cost of the Focused Strategy

The intermediate strategy $\alpha = \min(1, w_H/f)$ is the "Focused" strategy where High-Risk populations are prioritized. The total expected cost $c(\alpha)$ varies based on whether the specific capacity constraints for High-risk and Low-risk groups are met.
...
**Breakdown:**
1.  **Insufficient:** We use all $f$ on High-Risk. Zero reactive capacity. Cost is vaccination + all unprevented outbreaks in both groups.
2.  **Limited for L:** We saturate High-Risk ($w_H$). Remaining capacity ($f-w_H$) is used reactively on Low-Risk but is exhausted. Cost is total capacity $f$ + net unprevented outbreaks in Low-Risk.
3.  **Abundant for L:** We saturate High-Risk ($w_H$). Remaining capacity is sufficient to cover all Low-Risk outbreaks. Cost is pre-emptive ($w_H$) + reactive campaigns used ($w_L p_L$) + failed reactive attempts.
...
= p_{\mathrm{mean}}
  ...
  $$
    ...
    p_{\mathrm{rem}}(\rho, q)
  = p_{\mathrm{mean}}
    + \rho\bigl(p_{\mathrm{rem}}^{*}(q) - p_{\mathrm{mean}}\bigr),
    $$
...
f r - f p'(\alpha_*) + (1-\alpha_* f)\, p'_{\alpha}(\alpha_*) = 0.
...
Thus: 
  - If $r > p_{\mathrm{eff}}^{\mathrm{RL}}(\alpha)$ for all $\alpha$: **pure reactive**, $\alpha_* = 0$. 
  - If $r < p_{\mathrm{eff}}^{\mathrm{RL}}(\alpha)$ for all $\alpha$:
**pure pre-emptive**, $\alpha_* = 1$. 
- Otherwise:
**unique interior** $\alpha_*$ solving
$r = p_{\mathrm{eff}}^{\mathrm{RL}}(\alpha_*;\rho,\theta)$.
...
Thus: 
$$
q_*^{\mathrm{RL}} = (1-r)^2, \qquad
    \alpha_*^{\mathrm{RL}} = \frac{(1-r)^2}{f},

$$
  
subject to regime consistency ($f'(\alpha_*) < p'(\alpha_*)$) and $0 \le \alpha_* \le 1$.

This illustrates how strong heterogeneity and perfect targeting yield explicit closed forms.
For general $\theta$ and $\rho$, $\alpha_*$ satisfies the implicit analytic FOCs above.
...
Here: 
- $\omega$ is the **mixture weight** for the high–risk component, 
- $\theta_{\mathrm{L}}$ controls the baseline risk distribution (mostly lower risks), 
- $\theta_{\mathrm{H}}$ controls the high–risk tail (smaller $\theta_{\mathrm{H}}$ $\Rightarrow$ more mass near 1).

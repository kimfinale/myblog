...
우한에서 발생한 초기 일별 코로나19 환자수는 Kucharski et al. [@kucharski2020] 에 보고된 자료를 기반으로 하였다.
...
95% 신뢰 구간은 Log likelihood 가 asymptotically 아래의 조건을 만족한다는 사실을 이용하여 계산할 수 있다 (Wilks' theorem [@wilks1938]). $$ 2 (\mathrm {log} \mathcal {L} (\hat{\theta}) - \mathrm{log}\mathcal{L}(\theta_0)) \sim \chi^2_1$$

...
파티클 필터 (particle filter) 를 이용하여 잠재 변수 (latent variable)를 추정하는 과정을 지난 글에서 다루었다.  관찰값들이 코로나 19 일별 감염자일때 감염병 수리 모형을 이용하여 일별 감염재생산지수 ($(R_t$) 를 추정한다. 아래 글은 Kucharski et al. [@kucharski2020] 논문에 사용되었던 방법을 차용하였다. 파티클 필터 알고리즘의 기초는 Kitagawa [@kitagawa1996] 의 방법을 따른다. 이해를 돕기 위해 모형을 단순화 하였고 가상의 데이타를 만들어 내는 과정을 더하였다. 
